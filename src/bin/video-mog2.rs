...
fn main() -> Result<()> {
    ...
    // 高斯模糊去背景
    let mut mog = opencv::video::create_background_subtractor_mog2_def()?;
    ...
    loop {
        ...
        if let Ok(true) = cap.read(&mut frame) {
            // 去背景, 能够更好地识别阴影
            BackgroundSubtractorMOG2Trait::apply(&mut mog, &frame.clone(), &mut frame, -1.)?;
